  ...
  try {
    ...
    const recruitmentsWithCounts = await Promise.all(
      recruitments.map(async (rec) => {
        const totalApplicants = await Registration.countDocuments({ recruitment: rec._id });
...
router.get('/apply/:id', isAuthenticatedLineant, async (req, res) => {
  try {
...
router.post('/apply/:id', isAuthenticatedLineant, async (req, res) => {
  try {
    ...
    const alreadyApplied = await Registration.findOne({ recruitment: recruitmentId, email });
    if (alreadyApplied) {
      return res.render('applyRecruitment', {
        title: 'Apply for Recruitment',
        recruitment,
    ...
    const totalApplicants = await Registration.countDocuments({ recruitment: recruitmentId });
    res.render('applyRecruitment', {
      title: 'Apply for Recruitment',
      recruitment,
      user: req.user,
